...
class Knowledge {
    ...
    display_knowledge() {
        let agent1_kn = `${this.agent1.name}:   ${get_knowledge_to_print(
            this.get_knowledge(this.agent1)
        )}`
        let agent2_kn = `${this.agent2.name}:   ${get_knowledge_to_print(
            this.get_knowledge(this.agent2)
        )}`
        let self_kn = `Second Order:   ${get_knowledge_to_print(
            this.get_knowledge(this.me)
        )}`
        console.log(
            `${this.me.name} knows that:\n${agent1_kn}\n${agent2_kn}\n${self_kn}\n\n`
        )
    }
    ...
    get_best_agent(conceal = false) {
        ...
        for (let card_val = 1; card_val <= this.max_rank; card_val++) {
            ...
            if (have_card) my_amount = this.me.hand.how_many_of_value(card_val)
            if (conceal) {
                if (
                    have_card &&
                    open_card &&
                    temp_agent1 > 0 &&
                    temp_agent1 + my_amount > highest_agent1
                )
                    highest_agent1 = temp_agent1
                if (
                    have_card &&
                    open_card &&
                    temp_agent1 > 0 &&
                    temp_agent2 + my_amount > highest_agent2
                )
                    highest_agent2 = temp_agent2
            } else {
                if (
                    have_card &&
                    temp_agent1 > 0 &&
