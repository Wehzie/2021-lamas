...
class Knowledge{
    constructor(max_rank, agents, me) {
        ...
        this.kn_1 = new Array(this.max_rank).fill(-1)
        this.kn_2 = new Array(this.max_rank).fill(-1)
        this.second_order = new Array(this.max_rank).fill(-1)
    }

    update_knowledge(agent, card_value, amount){
    ...
    get_best_agent(max_rank){
        let highest_agent1 = 0
        let highest_agent2 = 0
        let temp_agent1 = 0
        let temp_agent2 = 0
        let have_card = false
        for (let card_val = 1; card_val <= max_rank; card_val++){
            temp_agent1 = this.kn_1[card_val - 1]
            temp_agent2 = this.kn_2[card_val - 1]
            have_card = this.me.has_specific_cards(card_val)
            if(have_card && temp_agent1 > highest_agent1) highest_agent1 = temp_agent1
            if(have_card && temp_agent2 > highest_agent2) highest_agent2 = temp_agent2
        }
