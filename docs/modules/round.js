...
class Round {
    ...
    draw_end_turn(me) {
        if (this.deck.draw_possible()) {
            ...
            display(
                "announce",
                `Round ${this.round_count}<br>${this.deck.size} cards left in deck`,
                false,
                1,
                false
            )
    ...
    player_single_turn(me, chosen_agent, chosen_card_value) {
        let query_message = `${me.name} asked ${
            chosen_agent.name
        } if they have any ${num2card_val(chosen_card_value)}s`
        ...
        if (chosen_agent.has_specific_cards(chosen_card_value)) {
            let chosen_card_amount =
                chosen_agent.hand.how_many_of_value(chosen_card_value)
            let action_message = `${
                chosen_agent.name
            } has ${chosen_card_amount} ${num2card_val(
                chosen_card_value
            )}'s and gives them to ${me.name}`
            display("logArea", action_message, true, 1)
            console.log(action_message)
            update_knowledge_all(
                me,
                this.agents,
                chosen_card_value,
                chosen_card_amount
            ...
            update_knowledge_all(
                ...
                this.agents,
                chosen_card_value,
                -1 * chosen_card_amount
            )
            // check whether a book was formed
            this.obtained_books += chosen_agent.give_cards(
                me,
                chosen_card_value
            )
        ...
        if (me.has_cards) {
            ...
            if (chosen_agent) {
                ...
                let chosen_card_value = me.choose_card_value(
                    this.num_of_card_sets,
                    chosen_agent
                )
                let action_message2 = `${me.name} asks ${
                    chosen_agent.name
                } if they have any ${num2card_val(chosen_card_value)}'s`
                ...
                if (chosen_agent.has_specific_cards(chosen_card_value)) {
                    ...
                    if (this.deck.draw_possible()) {
                        ...
                        display(
                            "logArea",
                            `${me.name} draws a card and finishes their turn`,
                            true,
                            1
                        )
                        ...
                        display(
                            "logArea",
                            `${me.name} finished their turn`,
                            true,
                            1
                        )
                        display(
                            "logArea",
                            `${me.name} no card can be drawn because the deck is empty`,
                            true,
                            1
                        )
